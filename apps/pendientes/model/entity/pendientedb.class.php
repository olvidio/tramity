...
class PendienteDB Extends core\ClasePropiedades {
	...
	function __construct($a_id='') {
		...
		if (is_array($a_id)) { 
			...
			if (isset($a_id) && $a_id !== '') {
				$this->iid_pendiente = intval($a_id); // evitem SQL injection fent cast a integer
	...
	public function DBGuardar() {
		...
		if ( core\is_true($aDades['cancilleria']) ) { $aDades['cancilleria']='true'; } else { $aDades['cancilleria']='false'; }
		...
		if ($bInsert === FALSE) {
			...
			$this->id_pendiente = $oDbl->lastInsertId('pendientes_id_pendiente_seq');
		}
	...
	/**
	 * Recupera tots els atributs de PendienteDB en un array
	 *
	 * @return array aDades
	 */
	function getTot() {
