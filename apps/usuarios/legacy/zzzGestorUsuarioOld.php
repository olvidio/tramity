 ...
 * @created 4/6/2020
 */
class zzzGestorUsuarioOld extends core\ClaseGestor
{
    /* ATRIBUTOS ----------------------------------------------------------------- */
    ...
    function getUsuariosQuery($sQuery = '')
    {
        ...
        if (($oDbl->query($sQuery)) === FALSE) {
            $sClauError = 'UsuarioRepository.query';
            $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClauError, __LINE__, __FILE__);
            return FALSE;
        }
    ...
    function getUsuarios($aWhere = array(), $aOperators = array())
    {
        ...
        if (($oDblSt = $oDbl->prepare($sQry)) === FALSE) {
            $sClauError = 'UsuarioRepository.llistar.prepare';
            $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClauError, __LINE__, __FILE__);
            return FALSE;
        }
        if (($oDblSt->execute($aWhere)) === FALSE) {
            $sClauError = 'UsuarioRepository.llistar.execute';
