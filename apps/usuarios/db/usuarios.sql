...
COMMENT
ON COLUMN public.aux_cargos.id_cargo IS 'corresponde al cargo';
COMMENT
ON COLUMN public.aux_cargos.id_ambito IS 'corresponde al tipo de instalacion: 1=cg, 2=cr, 3=dl, 4=ctr';
COMMENT
ON COLUMN public.aux_cargos.director IS 'director u oficial';
...
COMMENT
ON COLUMN public.aux_usuarios.id_cargo_preferido IS 'corresponde al cargo por defecto o preferido';
CREATE INDEX ON public.aux_usuarios ((lower (usuario)));
ALTER TABLE public.aux_usuarios OWNER TO tramity;

INSERT INTO public.aux_usuarios (usuario, id_cargo_preferido, password, nom_usuario)
