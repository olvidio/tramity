	...
	public function validatePost($aPOST){
		if (isset($aPOST['h'])) {
			...
			if (isset($aPOST['hh'])) {
				...
				$sUrl = $this->realFullUrl();
				if (!empty($aPOST)) {
					if (is_array($aPOST)) { array_walk($aPOST, 'core\poner_empty_on_null'); }
...
//					session_start();
	...
	static function link($sUrl_full) {	
		...
		if (is_array($aParam)) { array_walk($aParam, 'core\poner_empty_on_null'); }
	...
	private function ordenarQuery($sCampos) {
		...
		if (is_array($aCampos)) { array_walk($aCampos, 'core\poner_empty_on_null'); }
