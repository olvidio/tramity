...
class Desplegable
{
    ...
    function __construct($sNombre = '', $oOpciones = '', $sOpcion_sel = '', $bBlanco = '')
    {
        if (is_array($sNombre)) { //le puedo pasar los parámetros que quiera por el array
            ...
            if (isset($sNombre) && $sNombre !== '') {
                $this->sNombre = $sNombre;
            }
            if (isset($oOpciones) && $oOpciones !== '') {
                $this->oOpciones = $oOpciones;
            }
            if (isset($sOpcion_sel) && $sOpcion_sel !== '') {
                $this->sOpcion_sel = $sOpcion_sel;
            }
            if (isset($bBlanco) && $bBlanco !== '') {
    ...
    public function radio($num_col = '')
    {
        ...
        if (!empty($this->sOpcion_sel)) {
            if (is_array($this->sOpcion_sel)) {
                $a_sel = $this->sOpcion_sel;
            }
            if (is_string($this->sOpcion_sel)) {
                $a_sel = explode(",", $this->sOpcion_sel);
            }
        } else {
            $a_sel = [];
        }
    ...
    public function options()
    {
        ...
        if (is_object($this->oOpciones)) {
            ...
            foreach ($this->oOpciones as $row) {
                ...
                if (!empty($this->aOpcion_no) && is_array($this->aOpcion_no) && in_array($row[0], $this->aOpcion_no)) {
                    continue;
                }
            ...
            foreach ($this->oOpciones as $key => $val) {
                ...
                if (!empty($this->aOpcion_no) && in_array($key, $this->aOpcion_no)) {
                    continue;
                }
