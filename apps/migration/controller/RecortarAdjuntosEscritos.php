...
foreach ($oDblSt as $row) {
 ...
 $nom_fichero = $oEscritoAdjunto->getNom();
        ...
        switch ($i) {
            case 1:
                $oEscritoAdjunto->setAdjunto($contenido_doc1);
                $extension = $extension1;
                break;
            case 2:
                $oEscritoAdjunto->setAdjunto($contenido_doc2);
                $extension = $extension2;
                break;
            case 3:
                $oEscritoAdjunto->setAdjunto($contenido_doc3);
                $extension = $extension3;
                break;
            case 4:
                $oEscritoAdjunto->setAdjunto($contenido_doc4);
                $extension = $extension4;
                break;
            default:
                $oEscritoAdjunto->setAdjunto($contenido_doc5);
                $extension = $extension5;
                break;
        }
        // cambiar el nombre: prefix = mod_, extensión:
        $nom_fichero = 'mod_' . preg_replace('/\..*$/', ".$extension", $nom_fichero);
        $oEscritoAdjunto->setNom($nom_fichero);
