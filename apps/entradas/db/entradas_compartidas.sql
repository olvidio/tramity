...
CREATE INDEX IF NOT EXISTS entradas_compartidas_origen_idx ON nombre_del_esquema.entradas_compartidas USING GIN (json_prot_origen jsonb_path_ops);
CREATE INDEX IF NOT EXISTS entradas_compartidas_ref_idx ON nombre_del_esquema.entradas_compartidas USING GIN (json_prot_ref jsonb_path_ops);


--- adjuntos

CREATE TABLE nombre_del_esquema.entrada_compartida_adjuntos
(
...
ALTER TABLE nombre_del_esquema.entrada_compartida_adjuntos
    ADD CONSTRAINT entrada_compartida_adjuntos_fk_ent FOREIGN KEY (id_entrada_compartida) REFERENCES nombre_del_esquema.entradas_compartidas (id_entrada_compartida) ON DELETE CASCADE;
