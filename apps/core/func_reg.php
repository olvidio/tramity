...
/**
*
* Función para buscar el asunto de la oficina actual.
*
*/
function buscar_asunto_of($id_reg,$id_e_s,$cancilleria) {
	$oDBR=$GLOBALS['oDBR'];
	$asunto_of="";
	// cojo sólo el primer permiso
	$of_1=strtok(ConfigGlobal::permisos(),",");
	// para que la permanente vea el asunto oficina/s
	while ($of_1) {
		$sql_of="SELECT asunto_of
					FROM oficinas of JOIN x_oficinas u USING (id_oficina)
					WHERE of.id_reg=$id_reg AND of.id_e_s=$id_e_s AND cancilleria='$cancilleria' AND u.permiso='$of_1'
					";
		//echo "query: $sql_of<br>";
		$oDBRSt_q3=$oDBR->query($sql_of);
	
		if ($oDBRSt_q3->rowCount()) {
			$asunto_of.=$oDBRSt_q3->fetchColumn()." :: ";
		}
		$of_1=strtok(",");
	}
	return substr($asunto_of, 0, -4);
}
...
// referencias
	...
	foreach ($oDBR->query($sql_ref) as $row) {
		$j++;
		/*if ($cancilleria=="t" && $row["sigla"]!="IESE") {
			$ref_sigla="Cancillería (".$row["sigla"].")";
		} else {
			$ref_sigla=$row["sigla"];
		}*/
...
/**
*
* creo un array con los posibles estados...
*	primero el id (inglés), después el texto
*
*/
function array_status() {
	$opciones=array ( "NEEDS-ACTION" => _("iniciado"),
						"COMPLETED" =>  _("acabado"),
...
/**
*
* creo un array con los posibles modos de envio...
*	primero el id, después el texto
*
*/
function array_modo_envio() {
	$oDBR=$GLOBALS['oDBR'];
	$query_envio="SELECT id_modo_envio, modo_envio
					FROM x_modo_envio 
					ORDER BY orden";
	$i=0;
	foreach ($oDBR->query($query_envio) as $row_ctr) {
		$i++;
		$opciones[]=array ( 'id_modo_envio' => $row_ctr[0],
							'modo_envio' =>  $row_ctr[1] );
	}
	return $opciones;
}
/**
...
/**
*
* creo un array con los dias de la seman
