...
class EscritoForm
{
    ...
    public function render(): void
    {
        ...
        $url_update = 'src/escritos/controller/escrito_update.php';
        ...
        switch ($this->Q_filtro) {
            ...
            case 'distribuir':
                $pagina_cancel = Hash::link('src/expedientes/controller/expediente_distribuir.php?' . http_build_query($a_cosas));
                break;
            case 'enviar':
                $devolver = TRUE;
                $pagina_cancel = Hash::link('src/escritos/controller/escrito_lista.php?' . http_build_query($a_cosas));
                break;
            case 'en_buscar':
                $a_condicion = [];
                parse_str($this->str_condicion, $a_condicion);
                $a_condicion['filtro'] = $this->Q_filtro;
                $pagina_cancel = Hash::link('src/busquedas/controller/buscar_escrito.php?' . http_build_query($a_condicion));
                break;
            default:
                if ($this->Q_volver_a === 'expediente_ver') {
                    $pagina_cancel = Hash::link('src/expedientes/controller/expediente_ver.php?' . http_build_query($a_cosas));
                } else {
                    $pagina_cancel = Hash::link('src/expedientes/controller/expediente_form.php?' . http_build_query($a_cosas));
                }
        }
        ...
        $pagina_nueva = Hash::link('src/expedientes/controller/expediente_form.php?' . http_build_query(['filtro' => $this->Q_filtro]));
        ...
        $pagina_actualizar = Hash::link('src/escritos/controller/escrito_form.php?' . http_build_query($a_cosas));
