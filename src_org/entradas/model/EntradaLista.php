...
class EntradaLista
{
    ...
    */
    ...
    public function mostrarTabla(): void
    {
        ...
        $pagina_nueva = '';
        $ver_oficina = TRUE;
        $pagina_accion = ConfigGlobal::getWeb() . '/src/expedientes/controller/expediente_accion.php';
        switch ($filtro) {
            case 'en_encargado':
                $encargado = $this->aWhereADD['encargado'];
                $pagina_mod = ConfigGlobal::getWeb() . '/src/entradas/controller/entrada_ver.php';
                break;
            case 'en_aceptado':
                $oficina = $this->aWhereADD['ponente'];
                //$pagina_accion =  ConfigGlobal::getWeb().'/src/entradas/controller/entrada_accion.php';
                $pagina_accion = ConfigGlobal::getWeb() . '/src/expedientes/controller/expediente_accion.php';
                if ($_SESSION['oConfig']->getAmbito() === Cargo::AMBITO_CTR) {
                    $pagina_mod = ConfigGlobal::getWeb() . '/src/entradas/controller/entrada_form_ctr.php';
                    $ver_oficina = FALSE;
                } else {
                    $pagina_mod = ConfigGlobal::getWeb() . '/src/entradas/controller/entrada_ver.php';
                }
            ...
            case 'en_ingresado':
                $ver_oficina = FALSE;
                $pagina_mod = ConfigGlobal::getWeb() . '/src/entradas/controller/entrada_form.php';
                $pagina_nueva = Hash::link('src/entradas/controller/entrada_form.php?' . http_build_query(['filtro' => $filtro]));
                if (ConfigGlobal::role_actual() === 'vcd') {
                    $aQuery = ['filtro' => $filtro, 'slide_mode' => 't'];
                    $pagina_nueva = Hash::link('src/entradas/controller/entrada_lista.php?' . http_build_query($aQuery));
                }
                break;
            case 'en_admitido':
            case 'en_asignado':
            case 'entrada':
                ...
                break;
            case 'bypass':
                $pagina_mod = ConfigGlobal::getWeb() . '/src/entradas/controller/entrada_bypass.php';
                break;
            default:
                $pagina_mod = ConfigGlobal::getWeb() . '/src/entradas/controller/entrada_ver.php';
                $pagina_nueva = Hash::link('src/entradas/controller/entrada_form.php?' . http_build_query(['filtro' => $filtro]));
        ...
        $url_update = 'src/entradas/controller/entrada_update.php';
        ...
        $pagina_cancel = Hash::link('src/entradas/controller/entrada_lista.php?' . http_build_query($a_cosas));
        ...
        $pagina_cargar_dock = Hash::link('src/entradas/controller/entrada_dock.php?' . http_build_query(['filtro' => $filtro]));
        ...
        if ($this->slide_mode === 't') {
            ...
            include('src_org/entradas/controller/entrada_ver_slide.php');
        } else {
