...
foreach ($oDbl->query($sql) as $row) {
    ...
    switch ($tipo) {
        case 'bool':
            ...
            $exists .= "\n\t\t{";
            $exists .= "\n\t\t\t" . '$this->set' . $NomCamp . '(is_true($aDades[\'' . $nomcamp . '\']));';
            $exists .= "\n\t\t}";
            $ToEmpty .= "\n\t\t" . '$this->set' . $NomCamp . '(\'\');';
            break;
        case '_int8':
...
/* ESCRIURE LA CLASSSE ------------------------------------------------ */
$filename = ConfigGlobal::DIR . '/src/' . $grupo . '/model/entity/' . $Q_clase . '.php';
...
';
/* ESCRIURE LA CLASSSE ------------------------------------------------ */
$filename = ConfigGlobal::DIR . '/src/' . $grupo . '/model/entity/GestorBase' . $Q_clase . '.php';
