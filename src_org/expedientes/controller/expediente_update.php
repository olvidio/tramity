...
switch ($Q_que) {
    ...
    case 'guardar':
        ...
        if (!empty($error_txt)) {
            ...
            $pagina_mod = web\Hash::link('src/expedientes/controller/expediente_form.php?' . http_build_query($a_cosas));
    ...
    case 'cambio_vida':
        ...
        if (!empty($error_txt)) {
            ...
            $jsondata['id_expediente'] = $Q_id_expediente;
            $a_cosas = ['id_expediente' => $Q_id_expediente];
            $pagina_mod = web\Hash::link('src/expedientes/controller/expediente_ver.php?' . http_build_query($a_cosas));
            $jsondata['pagina_mod'] = $pagina_mod;
        }
        //Aunque el content-type no sea un problema en la mayoría de casos, es recomendable especificarlo
        header('Content-type: application/json; charset=utf-8');
