...
$id_cargo = array_search('scdl', $aPosiblesCargos, true);
if (!empty($id_cargo)) {
    // fijar reunión = 1;
    $filtro = 'fijar_reunion';
    $oExpedienteLista = new ExpedienteReunionFijarLista($filtro);
    $active = ($filtro === $Q_filtro) ? 'active' : '';
    $aQuery = ['filtro' => $filtro];
    $pag_lst = web\Hash::link('src/expedientes/controller/expediente_lista.php?' . http_build_query($aQuery));
    $num_orden = 1;
    ...
    // seguimiento = 2
    $filtro = 'seg_reunion';
    $oExpedienteLista = new ExpedienteReunionSeguimientoLista($filtro);
    $active = ($filtro === $Q_filtro) ? 'active' : '';
    $aQuery = ['filtro' => $filtro];
    $pag_lst = web\Hash::link('src/expedientes/controller/expediente_lista.php?' . http_build_query($aQuery));
    $num_orden = 2;
...
if (ConfigGlobal::mi_usuario_cargo() === 'scdl' || $perm_distribuir) {
    $filtro = 'distribuir';
    $oExpedienteLista = new ExpedienteDistribuirLista($filtro);
    $active = ($filtro === $Q_filtro) ? 'active' : '';
    $aQuery = ['filtro' => $filtro];
    $pag_lst = web\Hash::link('src/expedientes/controller/expediente_lista.php?' . http_build_query($aQuery));
    $num_orden = 3;
...
$pag_lst = web\Hash::link('src/escritos/controller/escrito_lista.php?' . http_build_query($aQuery));
$num_orden = 4;
$text = _("enviar");
...
$active = ($filtro === $Q_filtro) ? 'active' : '';
$aQuery = ['filtro' => $filtro];
$pag_lst = web\Hash::link('src/entradas/controller/entrada_lista.php?' . http_build_query($aQuery));
$num_orden = 7;
$text = _("E: introducir");
...
$active = ($filtro === $Q_filtro) ? 'active' : '';
$aQuery = ['filtro' => $filtro];
$pag_lst = web\Hash::link('src/busquedas/controller/buscar_escrito.php?' . http_build_query($aQuery));
$num_orden = 13;
...
if (ConfigGlobal::mi_usuario_cargo() === 'scdl') {
    ...
    $active = ($filtro === $Q_filtro) ? 'active' : '';
    $aQuery = ['filtro' => $filtro];
    $pag_lst = web\Hash::link('src/config/controller/parametros_scdl.php?' . http_build_query($aQuery));
    $num_orden = 90;
    $text = _("pref");
...
$pag_lst = web\Hash::link('src/pendientes/controller/pendiente_tabla.php?' . http_build_query($aQuery));
...
$pag_lst = web\Hash::link('src/escritos/controller/salida_escrito.php?' . http_build_query($aQuery));
$num_orden = 20;
...
$pag_lst = web\Hash::link('src/escritos/controller/mantenimiento.php?' . http_build_query($aQuery));
$num_orden = 21;
...
$pag_lst = web\Hash::link('src/plantillas/controller/plantilla_lista.php?' . http_build_query($aQuery));
$num_orden = 30;
$text = _("plantillas");
...
$pag_lst = web\Hash::link('src/busquedas/controller/imprimir_que.php?' . http_build_query($aQuery));
$num_orden = 40;
$text = _("imprimir");
...
$pagina_profile = web\Hash::link('src/usuarios/controller/personal.php?' . http_build_query([]));
$pagina_etiquetas = web\Hash::link('src/etiquetas/controller/etiqueta_lista.php?' . http_build_query([]));
$url_ajax = web\Hash::link('src/usuarios/controller/usuario_update.php');
